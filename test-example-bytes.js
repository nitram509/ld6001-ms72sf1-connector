...
function testExampleBytes() {
  ...
  const bytes = new Uint8Array([
    ...
    0x00, 0x00, 0x00, 0x00,
    // x, y, z (3 * 4 bytes)
    0x21, 0x28, 0x96, 0xBF, // x: -1.17 approx
    0x72, 0x6F, 0x81, 0xBF, // y: -1.01 approx? Image says 2.50. Wait.
    0xCB, 0x85, 0x20, 0x40, // z: 2.51 approx? Image says 0.31.
    // vx, vy, vz (3 * 4 bytes)
    0x8A, 0xBD, 0xC1, 0x3D, // vx: 0.094
    0x50, 0x98, 0x99, 0xBD, // vy: -0.074
    0x40, 0x52, 0xC3, 0x3A  // vz: 0.001
  ]);
  ...
  assert(Math.abs(p.z - (2.508)) < 0.001, `z mismatch: ${p.z}`);
